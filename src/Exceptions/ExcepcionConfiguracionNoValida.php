...
final class ExcepcionConfiguracionNoValida extends Exception
{
    ...
    {
        return new self('`TareaDelCambioDeBaseDeDatosDelInquilino` fails because `multitenencia.nombre_de_conexion_de_la_base_de_datos_del_inquilino` is `null` or equals to `multitenencia.propietario_database_connection_name`.');
    }

    public static function accionNoValida(string $actionName, string $configuredClass, string $actionClass): static
    {
        return new self("The class currently specified in the `multitenencia.actions.{$actionName}` key '{$configuredClass}' should be or extend `{$actionClass}`.");
    }
}
