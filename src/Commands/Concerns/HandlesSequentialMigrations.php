...
trait HandlesSequentialMigrations
{
    /**
     * Define el tipo de migración operando el entorno activo ('landlord' o 'tenant').
     */
    ...
    protected function getDatabaseConnection(): string
    {
        ...
        return match ($type) {
            'tenant' => config('multitenancy.tenant_database_connection_name', 'tenant'),
    ...
    {
        $type = $this->getMigrationType();
        $modules = MigrationPathHelper::getOrderedModulesInfo($type);

        if (empty($modules)) {
            $basePath = MigrationPathHelper::getBasePath($type);
        ...
        if (! $skipFilter && $this->hasOption('module') && $moduleFilter = $this->option('module')) {
            $filteredModules = MigrationPathHelper::findModulesByFilter($type, $moduleFilter);
    ...
    protected function displayAvailableModules(array $modules): void
    {
        ...
        $commandBase = $type === 'tenant' ? 'tenant' : 'landlord';
