...
use Spatie\Multitenancy\Commands\Concerns\HandlesSequentialMigrations;
use Spatie\Multitenancy\Support\MigrationPathHelper;

class TenantMigrateCommand extends Command
{
    use HandlesSequentialMigrations {
    ...
    public function handle()
    {
        ...
        if ($modules === null) {
            if ($this->option('show-modules') && ! empty(MigrationPathHelper::getOrderedModulesInfo($this->getMigrationType()))) {
                return 0;
            }
        ...
        foreach ($modules as $index => $module) {
            ...
            // Ejecutar el comando a través de tenants:artisan (soporte de spatie/laravel-multitenancy)
            $commandOptions = [
